...
public class CameraPath : MonoBehaviour
{
    ...
    [SerializeField] float defaultCamSize = 5;
    [SerializeField] EUpdateMethod updateMethod;
    ...
    [SerializeField] bool showTargetLocation = true;
    ...
    void CamUpdate()
    {
        ...
        Vector3 targetPos = (Vector3)GetClosestPoint(indexsToSearch) + new Vector3(CurrentSegment.Offset.x, CurrentSegment.Offset.y, zOffset);
        Vector3 smooth = Vector3.SmoothDamp(cameraTransform.position, targetPos, ref smoothVel, smoothing, Mathf.Infinity, Time.deltaTime);
        targetPos = Vector3.MoveTowards(cameraTransform.position, smooth, speed * Time.deltaTime);
        cameraTransform.position = targetPos;

        // Cam Size
        CameraPreset.DEFAULTCAMSIZE = defaultCamSize;
        float targetCamSize = cameraTrack[currentSegmentIndex].CameraSizeOverride;
        camera.orthographicSize = Mathf.MoveTowards(camera.orthographicSize, targetCamSize, zoomSpeed * Time.deltaTime);
    }
    ...
    public void JumpCamera()
    {
        ...
        cameraTransform.position = GetClosestPoint(indexsToSearch) + CurrentSegment.Offset;
        float targetCamSize = cameraTrack[currentSegmentIndex].CameraSizeOverride;
        camera.orthographicSize = targetCamSize;

        CameraPreset.DEFAULTCAMSIZE = defaultCamSize;
        camera.orthographicSize = cameraTrack[currentSegmentIndex].CameraSizeOverride;
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        if (showBounds)
        {
            foreach (CameraTrackSegement segment in cameraTrack)
            {
                segment.DrawBoarders(defaultCamSize);
            }
        }
    ...
    Vector2 GetClosestPoint(int[] searchArray)
    {
        ...
        Vector2 closestPoint = targetTransform.position;
...
{
    public static float DEFAULTCAMSIZE = 25;
...
public class CameraTrackSegement
{
    ...
    public void DrawBoarders(float defaultCamSize)
    {
        CameraPreset.DEFAULTCAMSIZE = defaultCamSize;
        if (!showBounds)
            return;
